...
vec4 effect( vec4 colour, Image texture, vec2 texture_coords, vec2 screen_coords )
{
    ...
    float fac2 = 1-(max(minOpacity, min(distance(uv.y, (aethereal.r - 1)), maxOpacity)));
    float maxfac = max(fac1, fac2);
    
    float low = min(tex.r, min(tex.g, tex.b));
    float high = max(tex.r, max(tex.g, tex.b));
	float delta = min(high, max(.5, 1. - low));

    // lower alpha and raise white to compensate
    tex.r = tex.r-delta + delta*maxfac*.3;
    tex.g = tex.g-delta + delta*maxfac*.3;
    tex.b = tex.b + delta*maxfac*1.9;
    tex.a = (1.-maxfac) * tex.a;

    float center_distance = .3 - distance(uv, vec2(.5, .5));
    if (center_distance > 0.)
    {
        // purple core
        tex.r = (1. - tex.r) * center_distance * .5 + tex.r;
        tex.g = tex.g - ((tex.g - 0.) * center_distance);
        tex.b = (1. - tex.b) * center_distance * .5 + tex.b;
    }
    float edge_distance = .2 - min(min(min(distance(uv.x, 0.), distance(uv.x, 1.)), distance(uv.y, 0.)), distance(uv.y, 1.));
